...
admin.setup = function (app, adminPrefix, username, password, projectDb, _sections) {
    ...
    app.get(adminPrefix, function (req, res) {
        ...
        _.forEach(sections, function (section) {
            output[section] = req.protocol + '://' + req.get('host') + adminPrefix + '/' + section;
        });
    ...
    app.post(adminPrefix + '/projects/apn', function (req, res) {
        ...
        if (!projectDb) {
            debug('POST /projects/apn', 'projectDb has not been setup properly');
    ...
    app.post(adminPrefix + '/projects/gcm', function (req, res) {
        ...
        if (!projectDb) {
            debug('POST /projects/gcm', 'projectDb has not been setup properly');
            return res.sendStatus(500);
        }
    ...
    app.post(adminPrefix + '/projects/wns', function (req, res) {
        ...
        if (!projectDb) {
            debug('POST /projects/wns', 'projectDb has not been setup properly');
            return res.sendStatus(500);
        }
    ...
    app.get(adminPrefix + '/projects/:projectType/:projectId', function (req, res) {
        if (!projectDb) {
            debug('GET /projects', 'projectDb has not been setup properly');
            return res.sendStatus(500);
        }

        projectDb.findProject(req.params.projectType, req.params.projectId, function (project) {
            if (project) {
                return res.send({
                    internal_id: project._id,
                    created: Math.floor(project.created.getTime() / 1000),
                    last_updated: Math.floor(project.last_updated.getTime() / 1000)
                });
            } else {
