...
class bdApi_ControllerAdmin_Log extends XenForo_ControllerAdmin_Abstract
{
	public function actionIndex()
	{
		...
		{
			$conditions['filter'] = array($filter['value'], empty($filter['prefix']) ? 'lr' : 'r');
			$filterView = true;
		}
		else
		{
	...
	public function actionDetail()
	{
		...
		$viewParams = array(
				'log' => $log,
				'client' => $this->getModelFromCache('bdApi_Model_Client')->getClientById($log['client_id']),
				'user' => $this->getModelFromCache('XenForo_Model_User')->getUserById($log['user_id']),
		);
